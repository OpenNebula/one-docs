...
Check that the workers publish their attributes with `onevm show`, and read `/var/log/one/oneflow.log` on the Front-end for the policy evaluation. OneFlow writes a `[AE] Checking policies for service: <id>` line for every service every 90 seconds. `oneflow show <service_id> --json` keeps the time of the last evaluation in `last_eval` of each policy.

The evaluation thread of OneFlow has died when the lines stop. It has also died when `last_eval` stays empty while the workers publish `SLURM_PENDING` above 0. `systemctl restart opennebula-flow` on the Front-end starts it again, and the service and its VMs are not affected. In OpenNebula 7.4 the thread dies when `oned` returns an empty VM monitoring pool. This happens after a Front-end outage longer than `VM_MONITORING_EXPIRATION_TIME` in `/etc/one/monitord.conf` (12 hours by default), and on a fresh installation before the first monitoring cycle. Nothing is written to `oneflow.log`, and the only trace is a `terminated with exception` line from `role.rb` in `journalctl -u opennebula-flow`. After a reboot of the Front-end, check for the `[AE]` line within three minutes.

The pool grows only while the workers publish `SLURM_PENDING` above 0, and only up to `max_vms`. `SLURM_PENDING` counts the jobs waiting for cores or memory that a worker of the role could give them. A job waiting with reason `PartitionConfig` requests more cores than the largest worker has, and never counts.

The pool shrinks only when the oldest worker publishes `OLDEST_IDLE=1`, plus the five minute cooldown of the policy. The worker publishes it after `ONEAPP_WORKER_IDLE_SECONDS` without a job and nothing pending, once it has drained its node. The node then shows as `drained` with reason `one-ondemand scale-down` in `sinfo -R`. A single worker never drains. If OneFlow does not remove a drained worker within `ONEAPP_WORKER_DRAIN_SECONDS`, the drain is undone.
...
OpenNebula does not resume the VMs on its own, and OneFlow shows the service in `WARNING`. Resume the storage VM, then the portal, then the workers. The service returns to `RUNNING` by itself, with its users and home directories intact.
