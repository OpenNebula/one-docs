...
OneFlow evaluates the elasticity policies on the average across the role. It adds one VM when `SLURM_PENDING` stays above 0 for two readings 30 seconds apart. It then waits 180 seconds before it reads again. That is long enough for the new worker to boot, so one pending job adds one worker. OneFlow evaluates the policies every 90 seconds by default (`autoscaler_interval` in `/etc/one/oneflow-server.conf` on the Front-end), so a pending job has its worker two to three minutes after it appears, and every further pending job about three minutes later. A job that no worker could run never counts, because Slurm refuses a GPU request at submit time on a pool without GPUs. A job that requests more cores than any worker has waits with the reason `PartitionConfig`, and the workers exclude it from `SLURM_PENDING`.
...
* Capacity for three VMs plus the workers you expect. The marketplace template gives every VM 2 vCPU and 4 GB of memory. The portal gets 8 GB. The VM template passes the CPU of the host through to the VMs (`CPU_MODEL` set to `host-passthrough`), so EESSI loads the software built for that CPU family. Every worker of a role has the same size, and a session cannot request more than one worker has. It runs the Slurm controller and MariaDB beside Open OnDemand, and used 710 MB of its 7941 MB with everything up on the testbed.
