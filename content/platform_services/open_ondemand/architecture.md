...
**Scaling up.** OneFlow adds one VM when `SLURM_PENDING` is above 0 in two consecutive readings, then waits 120 seconds before it reads again, so one pending job adds one worker and the new VM has time to boot. OneFlow reads the figures every `autoscaler_interval` seconds, 90 by default in `/etc/one/oneflow-server.conf` on the Front-end. With 30, as the [Requirements](#requirements) recommend, a pending job has its worker about 90 seconds after it appears and each further pending job about three and a half minutes later. Measured on the testbed with three 2 core jobs on a one worker pool, the second worker came 87 seconds after the submit, the job ran on it 50 seconds later, and the third worker came 217 seconds after the second. A job that no worker could ever run does not count. A GPU request on a pool without GPUs is refused at submit time, and a job that requests more cores than any worker has waits with the reason `PartitionConfig`, which the workers leave out of `SLURM_PENDING`.

**Scaling down.** OneFlow removes one worker at a time, always the oldest VM, and does not drain it first. So the oldest worker drains its own Slurm node after `ONEAPP_WORKER_IDLE_SECONDS` without a job (ten minutes by default) and with nothing in the queue. While that node is drained and empty, every worker publishes `OLDEST_IDLE` as `1`, and OneFlow removes the VM when the average is above 0.99 in two readings 60 seconds apart. The node leaves the cluster when the VM shuts down, and three guards apply. A worker never drains while the role is at `min_vms`. A drained node returns to service when a job appears in the queue, or when OneFlow has not removed it after `ONEAPP_WORKER_DRAIN_SECONDS` (ten minutes by default). A reconciler on the portal runs every 30 seconds and deletes any node whose VM has left the service and holds no job. On the testbed a scale down terminated exactly the drained VM, and the job on the other worker never left `RUNNING`.
