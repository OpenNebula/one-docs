...
* The portal and the storage role are one VM each, with no high availability. A reboot of the portal keeps the running and the pending jobs, because the queue, the nodes and the munge key live on the storage export, and the accounting database is dumped there every 30 minutes for a replaced portal (measured on 16 September 2026, munge, MariaDB, `slurmdbd`, `slurmctld` and the timers were active 32 seconds after the reboot and both jobs were still in the queue). A portal that OneFlow replaces by a new VM has not been exercised, the workers follow the new address from the service document and the key and the state come back from the export, both untested.
