...
A complete list of solved issues for 7.4.1 are listed in the [project development portal](https://github.com/OpenNebula/one/milestone/94?closed=1).
...
* Add new [OneDRS migration block option]({{% relref "product/cloud_system_administration/scheduler/drs/#blocking-vm-migrations" %}}) in Sunstone [#7783](https://github.com/OpenNebula/one/issues/7783).
* Allow [post-deployment VM group assignment]({{% relref "affinity#dynamic-vmg" %}}) in Sunstone [#4159](https://github.com/OpenNebula/one/issues/4159).
* New [Exec tab]({{% relref "product/virtual_machines_operation/virtual_machines/vm_instances.md#executing-a-command-from-sunstone" %}}) enables Virtual Machine Command Execution, enabling users to execute, monitor, retry, and cancel commands inside Virtual Machines and copy their output directly from Sunstone [#7559](https://github.com/OpenNebula/one/issues/7559).
* Include [permissions editing in update group form]({{% relref "manage_groups#manage-groups-permissions" %}}) in Sunstone [#6394](https://github.com/OpenNebula/one/issues/6394).
* New CLI option `--keep-ha` for `onezone serversync` command, which retains the local [RAFT configuration]({{% relref "frontend_ha.md#server-sync-ha" %}}).
* Add [option to log HA heartbeat]({{% relref "oned#configuration" %}}) and replication messages at log level 5 [#7977](https://github.com/OpenNebula/one/issues/7977).
* Add option to configure network lease policy for internal Address Ranges, set globally with option `REUSE_ADDRESS` in [oned.conf]({{% relref "oned#virtual-networks" %}}) [#7971](https://github.com/OpenNebula/one/issues/7971).
...
* Extend [Network PCI Physical Functions (PF) card control]({{% relref "product/cluster_configuration/pci_passthrough_sriov/network_interfaces#legacy-and-switchdev-modes" %}}) with link state management and flags in switchdev mode [#7679](https://github.com/OpenNebula/one/issues/7679).
* Extend link state management for network [Physical Functions when using Virtual Functions]({{% relref "product/cluster_configuration/pci_passthrough_sriov/network_interfaces#physical-functions-and-virtual-functions" %}}) as PCI network interfaces [#7679](https://github.com/OpenNebula/one/issues/7679).
...
* Fix opennebula-exporter VM data collection interrupted by malformed VM data [#7781](https://github.com/OpenNebula/one/issues/7781).
* Fix BSOD with Windows virtio injection [#7343](https://github.com/OpenNebula/one/issues/7343).
* Include OpenNebula API call errors in log with one-swap [#7334](https://github.com/OpenNebula/one/issues/7334).
* Append sudo to virt-v2v commands when using OneSwap [#7336](https://github.com/OpenNebula/one/issues/7336).
...
* Fix FireEdgeToken generation error for FireEdge remote authentication [#7730](https://github.com/OpenNebula/one/issues/7730).

## Updating Sunstone Configuration Files

With this release, some additional features have been added to Sunstone that require modification of the Sunstone configuration files to expose the new functionality. 

On the OpenNebula Front-end machine, locate the following files in the `/etc/one/fireedge/` and add the following configuration to the YAML files to expose the new functionality:

* File: `sunstone/views/*/vm-tab.yaml`

    * VM Group assignment of already-deployed VMs:
   ...
   * VM Command Execution in Sunstone:
...
* File: `sunstone/sunstone-server.conf`:

    * Modify the Sunstone date format:

        ```yaml
        dateFormat: 'dd/MM/yyyy'
        ```

* Files: `sunstone/views/admin/host-tab.yaml` and `sunstone/views/*/vm-tab.yaml`:

    * To add the Cluster filter for the Hosts and VMs view:
