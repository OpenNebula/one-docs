...
The `:server` section is passed directly to the Sinatra OneForm server. Any Sinatra setting supported by the version shipped with OpenNebula can be defined under this key. For the available settings, see the [Sinatra configuration documentation](https://sinatrarb.com/configuration.html). For example, besides `:bind` and `:port`, advanced deployments can configure `:host_authorization` in the same section.
