...
Cancellation safely stops resource provisioning while preserving the Provision context, allowing its resources to be deleted later.
...
To delete all infrastructure associated with a Provision and its corresponding OpenNebula resources, run:
...
If deprovisioning cannot be completed because of an uncontrolled error, you can remove only the Provision document:
...
{{< alert type="warning" >}}
**Important:** Use `--from-db` only as a last resort. It removes only the OneForm Provision document; it does **not** delete the OpenNebula objects or resources created at the external provider. An administrator must manually identify and remove every remaining resource in both environments.
{{< /alert >}}
