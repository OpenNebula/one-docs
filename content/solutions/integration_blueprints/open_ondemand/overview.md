...
* **Infrastructure Admin.** Runs the OpenNebula cloud and downloads the Open OnDemand Service from the Community Marketplace, which makes the image, the VM template and the service template available to every group. Also prepares what belongs to the site, which means the two Virtual Networks, the quotas of each VDC, the `autoscaler_interval` setting of the Front-end and, on a site with GPUs, the PCI passthrough configuration of the hosts.
...
The unit of isolation between groups is the service instance, not the portal. Inside one instance there is one user directory, one shared home and one Slurm cluster, and all its users share the same portal, workers and quotas, separated from each other as Unix users and Slurm jobs. Open OnDemand knows nothing about VDCs or OpenNebula groups. When two groups of a site, for example a department and an external project, must not share users, data or queue, the Tenant Operator of each group instantiates its own service in its own VDC. OpenNebula isolates them through the VDC, the networks and the quotas, and each group gets its own portal, users, home directories and Slurm cluster.
