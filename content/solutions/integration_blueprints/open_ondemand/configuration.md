...
This section describes the inputs of the Open OnDemand Service. It also covers role sizing, the worker pool, worker sizes, the Slurm Cluster, using an external Slurm Cluster, using an external identity provider and user management.
...
You can also set the Host name, the OpenID Connect inputs and the external Slurm attributes on a running portal with `onevm updateconf`. The portal reconfigures itself in under a minute. The portal keeps the certificate it already has for its Host name. A certificate switch changed later therefore takes effect only together with a new Host name. To apply it under the same Host name, remove `/etc/ood/ssl/<host_name>.crt` and `.key` on the portal. Without a Host name, the files are named after the address. The next reconfiguration issues the certificate again.
...
| `ONEAPP_POOL_RANGE` | derived | Address range the portal accepts workers from, as `first-last`. It also limits the number of Slurm nodes. Inside a service, the portal derives it from its compute interface, the whole `/24` subnet including its address. Set it only for a compute network larger than a `/24` subnet, or for a portal outside a OneFlow service. The range has to stay inside one `/24` subnet, because a session publishes the worker address that starts with the first three octets of the range. On a larger compute network, reserve a `/24` subnet slice of it for the workers and give that slice here. The portal role reads it. |
