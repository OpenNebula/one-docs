...
OpenNebula does not resume the VMs on its own, and OneFlow shows the service in `WARNING`. Resume the storage VM, then the portal, then the workers. The service returns to `RUNNING` by itself, with its users, home directories and Slurm queue intact. Then restart `opennebula-flow` on the Front-end, because its evaluation thread dies during the reboot, see [The Pool Does Not Scale](#the-pool-does-not-scale). A worker resumed before the portal joins the cluster once the portal answers, because its `slurmd` keeps retrying.
